...
The full annotated index (152 files, grouped by subsystem) lives in
...
- **Dashboard analytics reads stay REST-only** — `/usage`, `/usage/runs`, `/tags`, `/facets`, `/pivot`, `/activity`, `/read-activity`, `/read-ranking`, `/clusters` get no MCP tool and no CLI command: charts, not agent primitives, and most are name-bearing (scope-leak surface). Recorded as guarded `restOnly` fields in `telemetry-vocabulary.ts` (the spec pins the **nine** by name), so re-proposing one argues with a decision rather than filing a gap. `/clusters` has the strongest case of the nine: `lorekit dedupe` already answers the same question agent-side, over the WHOLE scope, from the identical clustering core, where the route only sees the newest `candidate_limit` rows. `/relevant` is NOT one of them — `memory.list order=rank` already covers it. [rationale](./docs/decisions.md#dashboard-analytics-reads-stay-rest-only)
...
- **Dashboard analytics live on one dedicated Insights page** — `/insights` composes `HealthSummary`, `UsageHealth`, `AgentBreakdown`, `ScopeConsumption`, `HotColdLore` and `RunsList` as named sections, reached from the sidebar's FIRST nav slot (`g i`) — the slot Overview holds while the flag is off, since exactly one of the two ever renders; `/settings/runs` is gone, moved there. Overview and the Explorer keep their original at-a-glance/editing jobs. Two independent range controls, not one page-wide picker: `HealthSummary`/`UsageHealth`/`AgentBreakdown` share ONE bounded-only `RangePicker` (`24h`/`7d`/`30d`/`90d`, no `all`) and ONE fetch (`lib/queries/insights-usage.ts`), which also fetches the immediately preceding equal-length window so `HealthSummary` can show week-over-week trend chips (`healthTrend`); `ScopeConsumption` keeps its OWN separate window (a leaderboard, not a diagnostic — forcing one range to answer both would silently narrow the other). `HealthSummary`/`UsageHealth` additionally exclude dashboard-originated calls (`excludeDashboardReads` in `lib/usage-health.ts`) — this page's tagline is "how your agents are actually using them", and browsing the Explorer yourself is not that (same principle as the Explorer's own dashboard-excluded read-activity cards, migration 00054). `AgentBreakdown`'s "who is reading" deliberately keeps the full row set, dashboard included, since showing that split IS its job. `HotColdLore`/`RunsList` stay account-wide/unwindowed. **The verdict is TWO-DIMENSIONAL** — `healthVerdict` grades reliability AND `readCoverage`'s records-per-read and reports the WORSE, with the banner naming the driver: success rate alone never moved (the `outcome` vocabulary has no "found nothing" state, so an empty-scope read is `ok`), so "100% of calls succeeded" occupied the page's first slot every day while the real signal sat three columns down. Coverage counts RECORD-BEARING tools only (`memory.list`/`read`/`search`) — `memory.write`/`org.*` are structurally `record_count: 0` — and reports `null` ("no reads"), never `0.0`, for a window with none. `healthTrend` needs `MIN_TREND_CALLS` (20) in the previous window, not merely non-zero: a one-call baseline renders "+99,900%". **Insights also inherits Overview's onboarding duties** — the flag hides `/overview`, which was the only host of `PendingInvitesBanner`/`OnboardingChecklist`/`GithubAppTeaser` and the only caller of `buildOnboardingSteps({ autoGenerateToken: true })` (the first-token mint), so all four moved to `insights/page.tsx` and the root redirect points at `/insights`. **Both leaderboards click through** to `/lore?scope=…` (and `&q=<key>` for a lesson); `ScopeConsumption`'s `unattributed` row stays inert, having no scope to narrow to. `RangePicker` takes a `label` prop so the page's two radiogroups do not share one accessible name. [rationale](./docs/decisions.md#dashboard-analytics-live-on-one-dedicated-insights-page)
