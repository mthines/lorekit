...
| `supabase/migrations/00043_usage_stats.sql` | `lorekit_usage_stats(p_user_id, p_since, p_until)` — SECURITY DEFINER + STABLE, aggregates `usage_events` (00034) into `(tool_name, outcome, scope_type, event_count, total_duration_ms)` over a half-open `[since, until)` window, sorted by count desc. Backs `GET /memories/usage`, the **read side** of usage analytics that 00034 never shipped (the table had an RLS select policy but nothing read it back as aggregates). Self-only visibility (`user_id = p_user_id`), same `p_user_id is null` + `auth.role() = 'service_role'` CI escape hatch (fails closed otherwise) and `plpgsql`-not-SQL ordering rationale as `lorekit_memory_scopes`. No `anon` grant. Proven by `migrations.test.sql`'s Usage-statistics section |
| `packages/mcp-core/src/usage-stats.ts` | Pure `parseUsageWindow` (period/`since`/`until` → validated `[since, until)`; rejects unknown period / unparseable / inverted window), `usageToolKind` (read/write/other classification of a `usage_events.tool_name`), `summarizeUsageRows` + `rollupByScopeType` (roll the grouped rows into the headline summary + per-scope-type view from the SAME rows — no second DB pass). Import-free, mirrored verbatim into `supabase/functions/_shared/usage-stats.ts` and guarded by `edge-parity.spec.ts` — the `ttl.ts` pattern. Consumed by `supabase/functions/memories/handlers/usage.ts` |
