...
- CI/CD is split: `ci.yml` validates (PRs + non-main branches, incl. a local-migration gate); `deploy.yml` owns `main` and is a **staging-first promotion pipeline** (test → deploy-staging → smoke-staging → deploy-production → smoke-production → rollback-on-failure). Two Supabase projects, secrets scoped via `staging`/`production` GitHub Environments. Migrations are forward-only (expand/contract + PITR); only Edge Functions auto-rollback. Do not re-merge these back into a single workflow — the split is what removed the old double test + double `db push` on every merge. See docs/deployment.md.
