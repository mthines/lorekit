...
- **The Explorer's Duplicate Clusters panel is a PANEL, not an instrument** — near-duplicate grouping over lesson bodies is not a `?filters=` dimension, so it never registers in `explorer-instruments.ts`/`filters.ts`; it lives in the Explorer (clicking a member ends with the reader holding lore) as a collapsed disclosure whose open state IS the React Query `enabled` flag, so a folded panel issues no request. Read-only — never merges, edits or deletes lore. **Two independent gates, in order:** the `lore-explorer-duplicate-clusters` flag decides whether the panel EXISTS (default `off`; `DuplicateClustersPanel{,.on,.off}.tsx` is the copy-and-suffix resolver, and the repo's live example of that convention), then the disclosure decides whether it FETCHES. `GET /memories/clusters` itself ships unflagged — additive and read-only. [rationale](./docs/decisions.md#dashboard-analytics-reads-stay-rest-only)
