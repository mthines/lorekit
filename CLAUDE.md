...
| `@lorekit/feature-flags` | `packages/feature-flags/` | OpenFeature-standard flag evaluation. `registry.ts` is the single hand-authored source of truth (zod-validated, all four OpenFeature value types — boolean/string/number/object); `nx run feature-flags:generate` projects it into typed TS bindings (`src/generated/flags.generated.ts`) and a language-neutral `generated/flags.manifest.json` for non-TS consumers. `LoreKitFlagProvider` resolves in order: session override → A/B(/n) experiment (deterministic FNV-1a bucketing, `bucketing.ts`, keyed on a stable `targetingKey`) → static default. Telemetry is two DIFFERENT mechanisms for two different questions: `featureFlagOtelHook` stamps `feature_flag.*` span attributes per evaluation (server-side, "what did this one evaluation resolve to"), while `packages/web`'s `FeatureFlagsProvider` tags every RUM signal with `feature_flag.<key>` = variant via `@dash0/sdk-web` (browser-side, "which flags were active for this visitor, retrospectively — Web Events"). UI-affecting experiments use a copy-and-suffix component convention (a resolver + one whole component per variant, never inline branches) — see the package's own `CLAUDE.md`. `/settings/developer` (the override UI) is open to any signed-in user outside production; in production it's gated by a hand-maintained email allowlist (`packages/web/src/lib/developer-users.ts`) enforced server-side (`notFound()` on the page itself), with the nav link's visibility separately toggled by a 5-clicks-on-the-avatar gesture (cosmetic only, never the access boundary). Package rules + file map: [`packages/feature-flags/CLAUDE.md`](./packages/feature-flags/CLAUDE.md). Full guide: [`docs/feature-flags.md`](./docs/feature-flags.md). Add/update/remove a flag via the `feature-flags` skill (`.claude/skills/feature-flags/SKILL.md`) |
| `@lorekit/web` | `packages/web/` | Next.js 15 dashboard (Vercel) |
| `@lorekit/cli` | `packages/cli/` | Zero-dep Node CLI. `install`/`uninstall`/`doctor` (scaffold the `lorekit-memory`/`lorekit-setup`/`lorekit-groom` skills + MCP server + lifecycle hooks into `.claude`; connectivity/token/scope health checks, now including a per-scope skill-version report via `shared/skill-versions.mjs`); `update` (offline refresh of the bundled skills + their hook command string to the shipped version — `doctor`'s outdated-skill warning and the SessionStart `updates.notify` drift nudge, `core/update-notify.mjs`, both point here; `--project`/`--global`/`--check`; CLI-only `native`, no catalog entry); read commands `list`/`search`/`show`/`stats`/`scopes`/`diff`/`tree`/`lint`/`dedupe`/`link` (Offline + Remote split, `--json`/`--scope`); `obligations` (checks a changed-file set — positionals/`--files`/stdin — against a declarative Surface-Partner Map of known, path-keyed file partnerships mined from existing CI guards, flagging any obliged partner not in the set; each entry declares a `state` (`advisory`/`gating`/`retired`, gating requiring an independent `guard`) and names the recurrence CLASS it instantiates (`src/shared/recurrence-clusters.mjs`); cwd-independent, CLI-only `native`, no catalog entry; `--json`/`--strict`/`--strict-all`); `invariants candidates` (compile pipeline's candidate scan — reuses `dedupe`'s Jaccard clustering, ranks clusters by summed `seen_count` × distinct scopes, and reports whether a cluster already resolves to a named recurrence class via the same join `dedupe` uses; never auto-compiles or gates, prints every memory a candidate would collapse for a human to hand-write into `obligations-map.mjs`; CLI-only `native`, no catalog entry; `--json`/`--scope`/`--min-seen-count`); maintenance sweeps `purge`/`purge-expired` (remote-only, account-wide, irreversible — confirm-or-`--yes`, refused for a scoped key); `hook` (the shared hook engine behind the plugins); `mcp` (local stdio MCP server); `migrate`; `completion` (zsh/fish completion scripts, generated from the command registry; dynamic scope/`scope::key` completion off the local store; installable via `install --completions <auto\|zsh\|fish\|none>`). Self-contained OTLP telemetry (`service.name=cli`). Full command reference: [`docs/cli.md`](./docs/cli.md) |
...
- **Lore Explorer filters through ONE two-level command menu + pills** — never one picker per dimension, never client-side narrowing; OR within a dimension, AND across; all dimensions filtered server-side; facets are their own drill-down query. **00110 refines the drill-down's label counts**: under `tags_mode='all'` (AND across the selected tags) a candidate label's count is now WITHIN-GROUP CO-OCCURRENCE — how many rows already satisfying every OTHER filter also carry every SELECTED tag *and* the candidate — where it used to be plain self-exclusion (any row carrying the candidate, whether or not it also carried the tags already picked); `tags_mode='any'` and every scalar dimension (kind/host/agent/trigger/repo/branch/pr/owner) are UNCHANGED self-exclusion. The facet-value enumeration is also now STABLE: every value is emitted from a full cross-join-then-filtered-count rather than filter-then-group, so a value with zero matching rows reports `count: 0` and stays listed instead of disappearing (AC-14 of the migration's own test block). [rationale](./docs/decisions.md#lore-explorer-filters-through-one-two-level-command-menu)
...
- **Dashboard analytics reads stay REST-only** — `/usage`, `/usage/runs`, `/tags`, `/facets`, `/pivot`, `/activity`, `/read-activity`, `/read-ranking`, `/utility`, `/clusters` get no MCP tool and no CLI command: charts, not agent primitives, and most are name-bearing (scope-leak surface). Recorded as guarded `restOnly` fields in `telemetry-vocabulary.ts` (the spec pins the **ten** by name), so re-proposing one argues with a decision rather than filing a gap. `/clusters` and `/utility` share the strongest case of the ten, both for the same reason — the agent-side spelling exists and is BETTER: `lorekit dedupe` already answers the clustering question over the WHOLE scope from the identical core, where the route only sees the newest `candidate_limit` rows; and `memory.list max_opened_count => 0` (00105) SELECTS the never-chosen lore over the whole scope, where `/utility` ranks a page of it and returns counts to paint. `/relevant` is NOT one of them — `memory.list order=rank` already covers it. [rationale](./docs/decisions.md#dashboard-analytics-reads-stay-rest-only)
- **The Explorer's Duplicate Clusters panel is a PANEL, not an instrument** — near-duplicate grouping over lesson bodies is not a `?filters=` dimension, so it never registers in `explorer-instruments.ts`/`filters.ts`. Read-only — never merges, edits or deletes lore. **Two independent gates, in order:** the `lore-explorer-duplicate-clusters` flag decides whether the surface EXISTS (default `off`; `DuplicateClustersPanel{,.on,.off}.tsx` is the copy-and-suffix resolver for the trigger, `DuplicateClustersSidebarPanel{,.on,.off}.tsx` its sibling for the sidebar body — one resolver per render site, so `LoreExplorer.tsx` carries no `&&` for either), then `open` (lifted to `LoreExplorer`, persisted via `PREFERENCE_KEYS.explorerClustersOpen`) decides whether the sidebar's query FETCHES — a closed trigger issues no request. **Trigger + non-modal sidebar, not an inline expanding panel:** `DuplicateClusters.tsx` is a compact bar that toggles `DuplicateClustersSidebar.tsx`, a LEFT column laid out as an ordinary flex sibling of the results card (`md:flex-row` in `LoreExplorer.tsx`) — never `position: fixed`, never a backdrop, so a reader can keep clicking lesson rows while it's open. **Selecting a cluster swaps the Explorer's own list**, not a nested detail pane: `handleSelectCluster` holds the picked cluster in state and `renderResults` shows exactly its members through the SAME `LessonCard` every other lesson renders with — one place to look at a lesson, not two. **Optimistic cache seeding** bridges the two: `lessonEntryFromClusterMember` (`lib/duplicate-clusters-view.ts`) builds a stand-in `LessonEntry` from the `hook` line the clusters response already carries, and `seedOptimisticLesson` (`lib/queries/lore.ts`) writes it into the SAME `['lesson-by-ref', scope, key]` cache slot `useLessonByRef` and the detail sheet read, then invalidates so the real row loads in behind it — the list shows something on the very first paint instead of a row of skeletons. Opening a member from the cluster view (`handleClusterMemberClick`) deliberately omits `openLessonById`'s `prefetched` argument — passing the still-optimistic stand-in there would freeze the detail sheet on partial data forever, since a prefetch is treated as fully resolved and never re-fetched. `GET /memories/clusters` itself ships unflagged — additive and read-only. [rationale](./docs/decisions.md#dashboard-analytics-reads-stay-rest-only)
- **Dashboard analytics live on one dedicated `/insights` route** — it composes `HealthSummary`, `UsageHealth`, `AgentBreakdown`, `ScopeConsumption`, `LoreUtilityGrid` (which replaced `HotColdLore`) and `RunsList` as named sections, led by `LoreCostHeadline`, reached from its own sidebar slot (`g i`); `/settings/runs` is gone, moved there. **`NAV` is ordered by the reader's journey — Explorer, Insights, Getting started** (find your lore → ask how it is used → set more up); the feature flag that used to gate this route and mutually-exclude it from the now-deleted Overview page has been removed entirely — `/insights` is unconditional. The Explorer (`/lore`) is the app's home slot and keeps its original find/edit job; `/insights` is analytics-only. Two independent range controls, not one page-wide picker: `HealthSummary`/`UsageHealth`/`AgentBreakdown` share ONE bounded-only `RangePicker` (`24h`/`7d`/`30d`/`90d`, no `all`) and ONE fetch (`lib/queries/insights-usage.ts`), which also fetches the immediately preceding equal-length window so `HealthSummary` can show week-over-week trend chips (`healthTrend`); `ScopeConsumption` keeps its OWN separate window (a leaderboard, not a diagnostic — forcing one range to answer both would silently narrow the other). `HealthSummary`/`UsageHealth` additionally exclude dashboard-originated calls (`excludeDashboardReads` in `lib/usage-health.ts`) — this page's tagline is "how your agents are actually using them", and browsing the Explorer yourself is not that (same principle as the Explorer's own dashboard-excluded read-activity cards, migration 00054). `AgentBreakdown`'s "who is reading" deliberately keeps the full row set, dashboard included, since showing that split IS its job. `LoreUtilityGrid`/`RunsList` stay account-wide/unwindowed (the cost line states its own rolling 30 days in the sentence itself). **The verdict is TWO-DIMENSIONAL** — `healthVerdict` grades reliability AND `readCoverage`'s records-per-read and reports the WORSE, with the banner naming the driver: success rate alone never moved (the `outcome` vocabulary has no "found nothing" state, so an empty-scope read is `ok`), so "100% of calls succeeded" occupied the page's first slot every day while the real signal sat three columns down. Coverage counts RECORD-BEARING tools only (`memory.list`/`read`/`search`) — `memory.write`/`org.*` are structurally `record_count: 0` — and reports `null` ("no reads"), never `0.0`, for a window with none. `healthTrend` needs `MIN_TREND_CALLS` (20) in the previous window, not merely non-zero: a one-call baseline renders "+99,900%". **Onboarding lives on `/lore`, not `/insights`** — `PendingInvitesBanner`/`OnboardingChecklist`/`GithubAppTeaser` and the `buildOnboardingSteps({ autoGenerateToken: true })` first-token mint render server-side above the extracted client Explorer on `/lore` (the app's home slot, `DEFAULT_POST_LOGIN_PATH`), self-hiding once irrelevant; `/insights` sheds them and renders only its `Suspense`-wrapped analytics. **The leaderboards click through** to `/lore?scope=…` (and `&q=<key>` for a lesson); `ScopeConsumption`'s `unattributed` row stays inert, having no scope to narrow to, and its rows WRAP below the `@md` CONTAINER breakpoint (bar onto its own line) so a long scope name gets the full width instead of a 128px column — a container query, not `sm:`, because the card is one column of a page grid and a wide screen never implied a wide card. A quadrant's **Copy prompt** copies a whole agent instruction assembled by the pure `lib/lore-utility-prompt.ts` — quadrant, thresholds, a CLOSED per-lesson action menu, a propose-then-wait rule, then the rows LAST — never the bare `scope::key` lines it replaced, which read identically under "prune these" and "promote these". The dashboard shell is the only scroller: `main` is `overscroll-contain` and `body:has([data-app-shell])` is `overflow:hidden; overscroll-behavior:none` in `globals.css`, so an overscroll cannot drag the whole shell off the page. `RangePicker` takes a `label` prop so the page's two radiogroups do not share one accessible name. [rationale](./docs/decisions.md#dashboard-analytics-live-on-one-dedicated-insights-route)
- **A COUNT must describe the rows its LIST returns — the retention thresholds reach all four readers** — the five conditions (`min_age_days`, `unseen_days`, `max_seen_count`, `max_read_count`, `max_opened_count`) moved `lorekit_memory_list` from 00092 onward and moved NOTHING else, so `max_opened_count => 0` narrowed the Explorer's rows while its facet counts, stat cards and matrix kept counting the un-narrowed population — every number on the page describing a different set from the one below it. 00108 gives `_facets`/`_activity`/`_pivot` the same five (plus a `created_since`/`created_until` pair on facets and pivot; activity's own `since`/`until` already bound `created_at`) through **ONE shared `lorekit_match_retention`** rather than a fourth inline copy — 00101 records that exact drift happening once already. The helper takes **cutoff INSTANTS, not day counts**, which is what keeps it `immutable` and index-usable (each caller resolves its `now()`-relative bounds once per query) and it holds 00100's never-opened rule in one place. It is applied in the `base` CTE's WHERE, **never as an `ok_*` flag**, so a threshold narrows even the SELF-EXCLUDED dimension — deliberately unlike a dimension, since self-exclusion exists so the dimension you are standing in still lists what you could switch to, and switching `host` does not stop you looking at lore older than 30 days. `q`/`key` stay unmirrored (a second `likeNeedle` in plpgsql). Client-side the mapping is likewise ONE function per layer (`_shared/api/retention.ts`, `retentionConditionsToAggregateBody`), and `?? null` is never a truthiness check — `max_opened_count => 0` is the whole point of 00105. The **Read** stat card stays scope-level: `usage_events` records a read's scope but nothing about the age or counters of the memories it returned, so a per-lesson threshold is unanswerable there. §108 pins the narrowing, the self-exclusion interaction, and that all-null params equal omitting them.
- **A read must never restamp `updated_at`** — `lorekit_record_memory_reads` (00099) bumped its counters with a plain UPDATE, which fired the `updated_at` trigger, so one 55-row `memory.list` page restamped 55 memories with no write of any kind. `updated_at` is the DEFAULT SORT for the Explorer and `memory.list order=recency`, so the ORDER AGENTS RECEIVED LORE IN was being driven by what had recently been read; on a 2,475-row sample nothing looked older than 14 days against a 22-day median. 00103 suppresses the trigger inside the read-recording function (not by dropping it, not by teaching each caller to restore the value). Read counters (`read_count`/`last_read_at`/`opened_count`/`last_opened_at`) move on reads; `updated_at` moves on writes; no column does both. §101 pins it. [rationale](./docs/decisions.md#a-read-must-never-restamp-updated_at)
- **Lore value is the RATIO `opened_count / read_count`, and `/insights` leads with the bill** — every absolute counter is supply-side (`seen_count` is 1 for 88.4% of rows; `read_count` is 99.80% bulk ride-alongs, so it ranks SCOPE BREADTH), and dividing cancels that confound. 00104 adds `opened_count`; 00106 adds `GET /memories/utility` (census + cost + per-quadrant rows, one `Promise.all`). **Five states, not four** — `load-bearing`/`specialist`/`noise-tax`/`dormant` from two booleans (chosen ≥ 2% pull-through, broad ≥ 100 deliveries) behind an evidence floor (age ≥ 7d AND deliveries ≥ 10), everything under it `unproven`; without the fifth a lesson written yesterday and one dead for a year render identically. A 0 is captioned with `counting_since`, never called "never". **One origin for the thresholds** — `LESSON_UTILITY_THRESHOLDS` in `@lorekit/schemas` is read by the TS chip AND passed INTO the SQL as parameters (the `lorekit_get_limit` posture; SQL defaults are a fallback, never the authority), so a chip and a quadrant count cannot disagree. **Two windows, deliberately** — the census reads LIFETIME counters on `memories` (it must agree with the chip), the cost sums `memory_read_daily` over a half-open `[since, until)` window; each surface captions its own. `LoreCostHeadline` states the estimate qualifier (4 chars/token, never tokenized) and that the share measures SELECTION, not influence — an injected lesson is already in context and needs no second fetch. `HotColdLore` is replaced; `/read-ranking` and `max_read_count` are NOT deprecated (they answer "what is this COSTING me in context"), and 00105's `max_opened_count => 0` is the condition the grooming UI's `max_read_count` placeholder of `0` was reaching for — on a live store `max_read_count <= 5` matched nothing and the first non-empty result came at 26. [rationale](./docs/decisions.md#lore-value-is-a-ratio-and-insights-leads-with-the-bill)
- **A citation is the agent's word, and it is a fact about a RUN** — pull-through measures whether a lesson was FETCHED, and a lesson injected at SessionStart is applied without ever being fetched, so it under-counts the dominant delivery path BY CONSTRUCTION and no read counter closes that gap. 00107 adds `cited: string[]` (`scope::key` refs) to `memory.write` / `POST /memories`, a `memory_citations` ledger, and `memories.cited_count` / `last_cited_at`. **A field, not a `memory.cite` verb** — every `tools/list` entry is context paid for in every session, and the retrospective write already happens where the agent knows what it applied. **Evidence, never a denominator** — citing is voluntary, so a `0` means "nothing said so", the opposite of `opened_count`'s zero; no rate, no share. **The run comes from `X-LoreKit-Correlation-Id`**, never the body, or a caller could attribute its citations to someone else's run. **The ref grammar is self-contained (`isReferenceScope`), deliberately NOT `validateScope`** — the two runtimes' copies are different strengths on purpose, so delegating would split `branch::o/r::feat/x::key` differently per surface and file one credit under two lessons (`memory-ref.spec.ts` caught exactly that). Every failure is SILENT (the write already committed); over-long lists TRUNCATE at 32; idempotency is `(cited, citing, coalesce(correlation_id,''))`, biased to under-count. The Stop hook names the session's shown-set in the ask so the agent picks rather than recalls. [rationale](./docs/decisions.md#a-citation-is-the-agents-word-and-it-is-a-fact-about-a-run)
- **`memory.read` batching (`refs: string[]`) is conditional, verbatim-scoped, and inflates a signal it doesn't count** — MCP, `POST /memories/read` and a variadic `lorekit show` let one call fetch several `scope::key` lessons instead of one `memory.read` per lesson. The response shape is CONDITIONAL on the input — singular `scope`+`key` keeps its exact pre-existing shape, `entries`/`missing` appears only under `refs` — which is a real cost paid to avoid a second tool: every caller now needs a shape-dispatch. Batch resolution matches the STORED scope VERBATIM (`groupRefsByScope`, no `validateScope` normalisation), because `memory_write` does no case-folding on insert and the transports differ in what they pass it (MCP lowercases via `validateScope`, REST's shape-only `RawScopeSchema` does not), so no reader can infer how a row was cased — normalising the batch read would make it look more correct than the data it reads. For the same reason `parseMemoryRefs` does not fold two refs differing only in scope case. Batching makes it cheap to open everything, which degrades `opened_count` pull-through (the exact ratio `/insights` is built on) since a shotgunned ref list costs the same as a considered one; the 32-ref truncation cap (shared with `cited`) bounds the damage, it does not remove it. `/insights`' `readCoverage` card shifts upward from this change and is not comparable across a window straddling the rollout — a known, accepted gap. [rationale](./docs/decisions.md#memoryread-batching-is-conditional-verbatim-scoped-and-inflates-a-signal-it-doesnt-count)
- **An omitted scope on a read means EVERYWHERE, not `global`** — `memory.read` / `memory.list` / `memory.list_archived` and `lorekit show` no longer require a scope. A `global` default was the obvious patch and is worse than the error it replaces: it turns a loud, recoverable `scope and key are required` into a silent `null` for every repo-scoped lesson, indistinguishable from a genuine miss (repo-scoped rows outnumber global ones ~2:1 on the store this was designed against). Instead an omitted scope resolves across every scope the caller can SEE — `memory.read` returns ONE winner, the two list tools simply widen. **Precedence is over scope TYPES, not the caller's own scopes**: the edge has no working directory behind an MCP call, so scope-type specificity (`project → branch → repo → global`, the hook engine's `readOrder`) is the one definition every runtime computes, in the import-free `scope/scope-precedence.ts`. Ties break by `updated_at` desc then scope asc — both load-bearing, since without a TOTAL order the winner is whatever order Postgres returned and the same call answers differently on consecutive runs. Three copies, two guards: the TS pair is byte-compared by `edge-parity.spec.ts` (hence import-free), the CLI's `.mjs` twin is held BEHAVIOURALLY by `scope-precedence-parity.spec.ts` — the failure it exists for is `lorekit show <key>` and `memory.read { key }` resolving one key to two different lessons with nothing to error about. A singular read ALWAYS reports the `scope` that answered (unconditional — never a second response shape, which this file already records regretting once) and names `other_scopes` only when the key was ambiguous; list entries each carry their own `scope`. Only the WINNER reaches `recordMemoryReads`, so a 50-candidate resolution does not inflate `read_count`. **REST's list routes are deliberately UNCHANGED** — already account-wide, and narrowing them to a winner would break the Lore Explorer; the CLI's remote `show` does the precedence pick client-side with the same shared rule. `lorekit.scope.type` stays OMITTED when unscoped; `lorekit.read.unscoped` / `lorekit.read.candidates` / `lorekit.list.unscoped` are the new measures. [rationale](./docs/decisions.md#an-omitted-scope-means-everywhere-not-global)
- **A tool's `inputSchema` carries NO top-level `oneOf`/`anyOf`/`allOf`** — Amazon Bedrock refuses any tool whose `input_schema` has one (`input_schema does not support oneOf, allOf, or anyOf at the top level`) and fails the **entire request**, not the one tool, so a single union costs every Bedrock-hosted client its whole `tools/list`. `memory.read` advertised one for its two exclusive argument shapes (`scope`+`key` xor `refs`) and took Agent0 down the day after #654 reached production. Mutually exclusive shapes go in the tool `description` (the half a model reads) and stay enforced in the handler; the schema stays permissive. There is no nested position that expresses exclusivity BETWEEN two top-level properties, so "move it lower" is not an option. Held by two gates at different layers: `JsonSchemaObject` omits the field (excess-property COMPILE error under the catalog's `satisfies`) and `tool-catalog-parity.spec.ts` asserts no tool's WIRE projection carries any of the three — the compile gate cannot see a schema smuggled past a cast. [rationale](./docs/decisions.md#a-tools-inputschema-carries-no-top-level-oneofanyofallof)
- **A skill's `metadata.version` must be bumped on any content change, and CI enforces it** — `doctor` / `lorekit update` / the SessionStart drift nudge compare the installed `SKILL.md`'s stamped version against the shipped one, a comparison that is a no-op if the stamp never moves; all three shipped skills sat at `1.0.0` through four content PRs before this guard existed. Deliberately version-based, not content-hash-based (a human decides whether a change is worth nudging users about) and not tied to the CLI package version (skill content and CLI releases are independent axes). `scripts/ci/skill-version-guard.mjs` is a real, non-advisory `ci.yml` job (`skill-version-guard`, in the `summary` job's required-check aggregation) that fails a PR touching a skill's files without bumping its version; it keys off the SOURCE `packages/cli/skill/*` dirs only — the plugin-vendored mirror is already guarded byte-for-byte by `sync-plugin-skill.mjs --check`, so the two compose rather than duplicate. [rationale](./docs/decisions.md#a-skills-metadataversion-must-be-bumped-on-any-content-change-and-ci-enforces-it)
