...
| `packages/mcp-core/src/audit.ts` | `AUDIT_ACTIONS`/`AuditAction`, pure `buildAuditEntry`, non-throwing `recordAudit` — mirrored self-contained in `supabase/functions/_shared/audit.ts`, THE single audit writer for the whole Deno edge tree (MCP tools **and** REST handlers). Not in `edge-parity.spec.ts`'s `MIRRORS`: the edge copy types the client as `ReturnType<typeof createClient>` off an `npm:` specifier where this one imports `SupabaseClient`, so (as with `limits.ts`) a whole-file comparison does not apply |
