...
- **The deploy SCOPE is measured against what is deployed** — each half is diffed against the SHA it last reached production at (`deployed/api-production` / `deployed/web-production`), never against the previous commit; the tags fail open, and the decision is the unit-tested `scripts/resolve-deploy-scope.mjs`. Never reinstate the single-push baseline: it let the web be promoted ahead of an API that had never deployed. [rationale](./docs/decisions.md#cicd-is-split-ciyml-verifies-deployyml-promotes)
