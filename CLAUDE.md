...
- **Dashboard analytics live on one dedicated Insights page** — `/insights` composes `HealthSummary`, `UsageHealth`, `AgentBreakdown`, `ScopeConsumption`, `HotColdLore` and `RunsList` as named sections, reached from the sidebar's fourth nav slot (`g i`); `/settings/runs` is gone, moved there. Overview and the Explorer keep their original at-a-glance/editing jobs. Two independent range controls, not one page-wide picker: `HealthSummary`/`UsageHealth`/`AgentBreakdown` share ONE bounded-only `RangePicker` (`24h`/`7d`/`30d`/`90d`, no `all`) and ONE fetch (`lib/queries/insights-usage.ts`), which also fetches the immediately preceding equal-length window so `HealthSummary` can show week-over-week trend chips (`healthTrend`); `ScopeConsumption` keeps its OWN separate window (a leaderboard, not a diagnostic — forcing one range to answer both would silently narrow the other). `HealthSummary`/`UsageHealth` additionally exclude dashboard-originated calls (`excludeDashboardReads` in `lib/usage-health.ts`) — this page's tagline is "how your agents are actually using them", and browsing the Explorer yourself is not that (same principle as the Explorer's own dashboard-excluded read-activity cards, migration 00054). `AgentBreakdown`'s "who is reading" deliberately keeps the full row set, dashboard included, since showing that split IS its job. `HotColdLore`/`RunsList` stay account-wide/unwindowed. [rationale](./docs/decisions.md#dashboard-analytics-live-on-one-dedicated-insights-page)
