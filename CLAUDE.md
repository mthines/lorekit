...
- `lorekit.mcp.client.name` / `.source` / `.version` — WHICH MCP host is calling (on the root `lorekit.mcp` span for every method; `usage_events.mcp_client` on every `tools/call`). Bounded: `claude-code|claude-desktop|agent0|cursor|windsurf|vscode|cline|continue|zed|lorekit-cli|mcp-remote|mcp-inspector|other`, and OMITTED when nothing identified the caller. Resolved by the shared `mcp-client-attribute.ts` (mirrored into `_shared/`), never by an inline header sniff. `.source` (`client_info|user_agent`) records which input answered; `.version` rides on `initialize` ALONE — it is the one pass-through value, so a per-request version would put an open-ended dimension on every span. Read both inputs, not just the handshake: the server is stateless, so `clientInfo` arrives once and `User-Agent` is the only per-request signal. See Key decisions and [docs/otel.md](./docs/otel.md) → "Which MCP client is calling"
...
- **Which MCP client is calling is a BOUNDED, LoreKit-namespaced dimension read from TWO inputs** — `memory.read`'s top-level `oneOf` made Bedrock reject the WHOLE `tools/list` (not just that tool), and a second client dropped `memory.read` alone from a 22-tool list; **neither produced an error here** — we answered 200 with a valid result and the rejection happened in the host's own process, so the last event in our system was a success. Nothing recovers the upstream error (there is no callback), so only PREVENTION stops the class; what identity buys is that the next regression reads as a chart with a step in it. `lorekit.mcp.client.name`/`.source`/`.version` on the root span for every method + `usage_events.mcp_client` on every `tools/call` (00110). Read from `clientInfo` AND `User-Agent` **per request**, because the server is stateless — `clientInfo` arrives on `initialize` ALONE, so handshake-only labelling leaves every tool-call span unlabelled, which is exactly the per-tool question the dimension exists for; `source` records which input answered so a record is distinguishable from a hint. Output is always from the closed vocabulary (both inputs are caller free text), with the two absence rules ASYMMETRIC on purpose: an unrecognised `clientInfo` is `other` (a countable unknown client), an unrecognised `User-Agent` is OMITTED (a runtime default identifies nothing, and would pollute that bucket). `.version` is the one pass-through value — allowlisted, capped at 32, `initialize` only. `lorekit.*` not `mcp.*` because semconv 1.43.0 defines no client-identity attribute and spends `mcp.client.*` on client-role METRIC names; `mcp_client` not `client` because `client` is already the SURFACE (00054) and `host` a bucket's owning host (00056). Null on the REST surface is CORRECT and must stay representable. A `tools/list` usage event and the list→call funnel alert are deliberately NOT part of this. [rationale](./docs/decisions.md#which-mcp-client-is-calling-is-a-bounded-lorekit-namespaced-dimension-read-from-two-inputs)
