...
- **Who sends it.** The CLI attaches `traceparent` to every outgoing REST call (`restFetch`)
  and every MCP call (`mcpCall` — all four `org.*` ops plus the `ping` fallback; every memory
  operation, hard-delete included, is REST now), sourced
