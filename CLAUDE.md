...
| `@lorekit/cli` | `packages/cli/` | Zero-dep Node CLI. `install`/`uninstall`/`doctor` (scaffold the `lorekit-memory`/`lorekit-setup`/`lorekit-groom` skills + MCP server + lifecycle hooks into `.claude`; connectivity/token/scope health checks); read commands `list`/`search`/`show`/`stats`/`scopes`/`diff`/`tree`/`lint`/`dedupe`/`link` (Offline + Remote split, `--json`/`--scope`); `obligations` (checks a changed-file set — positionals/`--files`/stdin — against a declarative Surface-Partner Map of known, path-keyed file partnerships mined from existing CI guards, flagging any obliged partner not in the set; cwd-independent, CLI-only `native`, no catalog entry; `--json`/`--strict`); maintenance sweeps `purge`/`purge-expired` (remote-only, account-wide, irreversible — confirm-or-`--yes`, refused for a scoped key); `hook` (the shared hook engine behind the plugins); `mcp` (local stdio MCP server); `migrate`. Self-contained OTLP telemetry (`service.name=cli`). Full command reference: [`docs/cli.md`](./docs/cli.md) |
