...
- **Dashboard analytics live on one dedicated Insights page** — `/insights` composes `UsageHealth`, `AgentBreakdown`, `ScopeConsumption`, `HotColdLore` and `RunsList` as named sections, reached from the sidebar's fourth nav slot (`g i`); `/settings/runs` is gone, moved there. Overview and the Explorer keep their original at-a-glance/editing jobs. Not every section shares one range control — only `ScopeConsumption` has a real window, so only it gets a (local) `RangePicker`. [rationale](./docs/decisions.md#dashboard-analytics-live-on-one-dedicated-insights-page)
- **MCP clients authenticate by OAuth 2.1 (authorization code + PKCE), and the authorization server is the Next.js dashboard, not an Edge Function.** An OAuth-issued credential is just an `api_tokens` row (`kind='oauth'`, `client_id`, a 30-day `expires_at` enforced at auth time on both `mcp/auth.ts` and `_shared/api/auth.ts`) — it reuses the org/scope scoping columns and `KeyRestriction` enforcement 00068/00069 already added, never a second scoping mechanism: the consent screen's per-org checkboxes map onto `org_access='selected'` + `org_ids`, empty selection maps onto `org_access='personal'`, and every `memory.*` tool call already threads `keyScoping` through `applyTenantScope` and the `memory_write`/`memory_delete` RPCs with no OAuth-specific code path. The MCP endpoint on `*.supabase.co` stays the RESOURCE server; `lorekit.io` is the AUTHORIZATION server and serves both discovery documents (RFC 9728/8414/7591), because the consent screen needs the Supabase-Auth session and `api_tokens` write path that already live in `packages/web`. A credential-less MCP request now gets `401` + `WWW-Authenticate` (the OAuth discovery trigger); an invalid/expired/rotated one still gets an in-band JSON-RPC `-32001` echoing the real request id, because a `401` there makes streamable-HTTP clients retry silently and hang. [rationale](./docs/decisions.md#mcp-oauth-21-authorization)
