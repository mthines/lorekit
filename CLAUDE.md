...
| `@lorekit/feature-flags` | `packages/feature-flags/` | OpenFeature-standard flag evaluation. `registry.ts` is the single hand-authored source of truth (zod-validated); `nx run feature-flags:generate` projects it into typed TS bindings (`src/generated/flags.generated.ts`) and a language-neutral `generated/flags.manifest.json` for non-TS consumers. `LoreKitFlagProvider` resolves flags from the registry; A/B experiments use deterministic FNV-1a bucketing (`bucketing.ts`) keyed on a stable `targetingKey`. `featureFlagOtelHook` stamps `feature_flag.*` span attributes + a `lorekit.feature_flag.evaluations` counter on every evaluation, so experiment conversion is a Dash0 query away. Full guide: [`docs/feature-flags.md`](./docs/feature-flags.md) |
