...
| `packages/schemas/src/audit.ts` | **The** `audit_log.action` vocabulary: `AUDIT_ACTIONS` (24 actions), `AuditActionSchema`, and the `AuditEntryInput`/`AuditRow` shapes. Single source of truth for the Node writer, the Deno writer, the dashboard's badge metadata, and — restated in SQL, drift-guarded — the CHECK constraint. Replaces four divergent copies (11 / 11 / 24 / 23 actions) |
| `packages/mcp-core/src/audit.ts` | Node writer: pure `buildAuditEntry`, non-throwing `recordAudit`. Re-exports the vocabulary + types from `@lorekit/schemas/audit` rather than re-declaring them. Deno counterpart is `supabase/functions/_shared/audit.ts` (the `limits.ts` mirror pattern, but the action list is now shared, not duplicated) |
| `supabase/functions/_shared/audit.ts` | **Canonical Deno audit writer**, shared by MCP and REST. `buildAuditEntry` + `recordAudit`, plus `recordRestAudit(db, span, auth, input)` — the REST wrapper that resolves the actor as `auth.userId ?? null`, opens a `lorekit.rest.audit` child span (bounded `lorekit.audit.action` attribute), and never throws. Types come type-only from `@lorekit/schemas/audit`, so the edge bundles gain no zod for it. `supabase/functions/mcp/audit.ts` is now a thin re-export of this module |
| `packages/mcp-core/src/audit-actions-drift.spec.ts` | Source-scan drift guard: parses the action list out of the LATEST `audit_log` action CHECK migration (discovered by number, never hardcoded) and asserts it equals `AUDIT_ACTIONS` as a set; also asserts `AUDIT_ACTION_META` has exactly one entry per action. Anti-vacuity floors on every parsed list |
| `packages/mcp-core/src/rest-audit-usage.spec.ts` | Source-scan drift guard: derives the mutating REST handler set from the route tables in `memories/index.ts` + `orgs/index.ts` (parsed, not hardcoded — POST/PATCH/DELETE excluding `requires: 'read'`), resolves each handler to its file via the index's imports, and asserts each calls `recordRestAudit` imported from `_shared/audit.ts`. `AUDIT_EXEMPT` is empty by design |
| `supabase/migrations/00040_audit_log_github_app_action.sql` | Forward-only drop+re-add of the `audit_log.action` CHECK, widened with `github_app.installation_linked` — fixing real, silent audit loss (web's `handleSetupReturn` emitted it, the 00027 CHECK rejected it, the non-throwing writer swallowed the error). The resulting list is exactly `AUDIT_ACTIONS`. Same pattern as 00023 / 00027 |
...
| `packages/web/src/lib/audit-actions.ts` | `AUDIT_ACTION_META` single-record badge metadata (labels/icons/colours). `AUDIT_ACTIONS`/`AuditAction` are re-exported from `@lorekit/schemas/audit`, no longer re-declared — the presentation layer stays here so the schemas package never gains `lucide-react` |
...
- Audit logging (`audit_log` table, `supabase/migrations/00010_audit_log.sql`) is captured **at the app layer** — an explicit `recordAudit`/`recordAuditEvent` call right after each sensitive mutation succeeds (API-key create/revoke, webhook-secret create/rotate, memory create/update/archive/restore/delete), not by DB triggers on the underlying tables — the app layer can resolve the actor (`auth.uid()` for dashboard actions, the resolved token/user for MCP, `null` for service-role/CI) and shape a human-readable `target`/`metadata` that a trigger on `memories`/`api_tokens`/`webhook_secrets` cannot. **The one deliberate exception is `user_limits`**: no app-layer code path writes that table today (raising a limit is a raw SQL upsert), so there's no call site to instrument — an `AFTER INSERT OR UPDATE OR DELETE` DB trigger (`audit_user_limits()`) is the only way to capture `limit.override` events, mirroring the `enforce_memory_cap()` DB-trigger precedent for the same reason (the write-path actor isn't app-visible there either). `recordAudit`/`recordAuditEvent` never throw — a failed audit write must never break the primary operation it's auditing. **The REST edge functions audit too, as of the `_shared/audit.ts` change** — every mutating route in `memories/` and `orgs/` calls `recordRestAudit` after its operation succeeds (never on an error/404/zero-match path), so migrating a client from MCP or direct-Supabase onto REST no longer silently turns the audit trail off. The Deno writer lives ONCE, in `supabase/functions/_shared/audit.ts`; `supabase/functions/mcp/audit.ts` is a thin re-export of it (the `mcp` function already cross-imports `_shared/otel.ts`, so this crosses no boundary it did not already cross) — this is deliberately NOT the `limits.ts` self-contained-mirror pattern, because both edge surfaces needed the same writer and two copies had already drifted. **The action vocabulary lives in exactly one hand-maintained place**, `packages/schemas/src/audit.ts` (`AUDIT_ACTIONS`), consumed by the Node writer, the Deno writer and the dashboard; Postgres cannot import it, so the `audit_log.action` CHECK is a second, enforcing copy kept honest by `audit-actions-drift.spec.ts` (parses the latest CHECK migration, compares as a set). Adding an action is always two edits — the tuple plus a forward-only drop-and-re-add CHECK migration. That seam had already failed silently once: web's `handleSetupReturn` emitted `github_app.installation_linked`, the CHECK rejected it, and the non-throwing writer swallowed the error, losing every such event until `00040_audit_log_github_app_action.sql`. **Actor resolution differs between the surfaces and REST's is strictly better:** `AuthContext.userId` is populated for JWT users, so `recordRestAudit` records the real actor and the `user_id = auth.uid()` INSERT policy is satisfied under the RLS-scoped client — whereas MCP's `getUserId` returns `null` for JWT users, so those MCP audits fail RLS and are swallowed (a documented MCP limitation). Two executable drift guards keep this from regressing: `audit-actions-drift.spec.ts` (vocabulary) and `rest-audit-usage.spec.ts` (every mutating REST route, derived from the route tables, must call `recordRestAudit`).
