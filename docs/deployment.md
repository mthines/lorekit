...
> `LOREKIT_APP_URL` is the dashboard origin the MCP server points clients at.
> It defaults to `https://lorekit.io`. Optional for the cap/rate-limit message
> links it was introduced for — but **load-bearing for OAuth on any non-production
> project**: it is the origin in the `WWW-Authenticate` challenge and in the
> protected-resource redirect, so a staging deployment without it challenges
