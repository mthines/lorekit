...
see `packages/mcp-core/src/audit/audit.ts` and CLAUDE.md's "Key decisions". Because
...
| Cap error translation | `supabase/functions/mcp/limits.ts` → wired in `tools.ts` (`toolWrite`) | `packages/mcp-core/src/limits/limits.ts` (`LimitError`, `translateCapError`) |
| Rate-limit check + 429 | `supabase/functions/mcp/index.ts` (post-`resolveAuth`) | `packages/mcp-core/src/limits/limits.ts` (`checkRateLimit`) |
