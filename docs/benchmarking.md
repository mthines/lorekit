...
# 2 rps/user, so 20 rps needs 10 — the script refuses fewer.
  … node scripts/load-test/load-test.mjs --target preview --surface mcp --rps 20 --users 10

# The CLI's remote path (REST + lk_ token — the api_key tier):
  … node scripts/load-test/load-test.mjs --target preview --surface rest --auth token

# Stress: step 20 -> 160 rps and report the highest sustained rate.
  … node scripts/load-test/load-test.mjs --target preview --surface mcp --ramp --rps 20 --max-rps 160 --users 80
```
