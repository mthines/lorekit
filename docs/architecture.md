...
The catalog is **zero-import by construction** — a `zod` or relative import
would break both the edge mirror and the generator's bare-checkout contract — so
it holds names and data, never references. Consumers reach it one of three ways,
and the tier is chosen by what the consumer *can* do, not by preference:

| Tier | When | Examples |
|------|------|----------|
| **Derive** | The consumer can import the catalog. No generator, no staleness surface, nothing to check. | `mcp-core` specs, `llms/render.ts` (tool reference + permission matrix), the edge's `wireTools()` for `tools/list` |
| **Generate** | The consumer *cannot* import it. Committed artifacts, regenerated by `node scripts/codegen/gen-surfaces.mjs`, staleness caught by `--check`. | `packages/cli/src/surfaces.generated.mjs` (the CLI is a published **zero-dependency** package, so importing the workspace schemas would break the artifact) · `supabase/functions/mcp/tool-dispatch.generated.ts` (a dispatch map must bind a name to an imported FUNCTION, which a zero-import catalog cannot hold) |
| **Assert** | Deriving would be wrong. Two independent statements are held together by a test instead. | `permissions.ts`'s `READ_TOOLS`/`WRITE_TOOLS` — the duplication IS the authorization control, so it stays hand-written and cross-checked (`tool-catalog-parity.spec.ts`) · editorial prose in `llms/template.md` |

**Never hand-edit a `*.generated.*` file.** Edit the catalog and regenerate;
`gen-surfaces.mjs --check` fails CI when the two disagree, and
`surface-generator.spec.ts` proves that gate bites by perturbing a sandboxed
copy. CLI *behaviour* stays hand-written in `packages/cli/src/commands.mjs` —
the one registry `bin/lorekit.mjs` derives its command table, aliases, flags,
help and tracing from — so the catalog decides the surface and the registry
decides the behaviour.

Two more gates cover what generation cannot: `packages/cli/test/surface-parity.test.mjs`
checks the CLI half of the bijection (it lives in the CLI's own suite, not
`mcp-core`, because Nx would otherwise cache-hit a spec that does not depend on
the files it polices), and `deno check` over the edge tree — the only gate in
the repo that sees an **arity** change to a handler, which no name-based scan can.
...
its holder has left. Tables and columns: `supabase/migrations/00095_oauth.sql`.
...
absent from the agent surface by decision, not by omission (see
[decisions.md](./decisions.md#dashboard-analytics-reads-stay-rest-only)).
