...
| Retention policies and grooming — `policy.list/create/update/delete`, `groom.preview/run` on MCP; `GET/POST /policies`, `PATCH/DELETE /policies/:id`, `POST /groom/preview`, `POST /groom/run` on REST | **Gated the same way `memory.*` is, on both surfaces.** `policy.create` / `POST /policies` refuses (MCP `-32003`, REST `403`) a target `scope` outside the allowlist. `policy.update` / `policy.delete` and their REST twins gate the policy's **stored** scope (fetched before the mutation, not the request body — a policy's scope cannot be changed by an update) — a scope-restricted token cannot rename or delete a policy fenced outside its allowlist, even by id. `policy.list` / `GET /policies` **narrows**: a fenced-scope policy is silently dropped rather than refusing the whole list. `groom.preview` / `groom.run` and their REST twins gate the RESOLVED scope — an inline `scope`, or (via `policy_id`) the referenced policy's stored scope — so a scoped token cannot preview or archive against a saved policy it could not itself have created. Before this, these seven operations only checked read/write PERMISSION, not the scope allowlist — a scoped token could create, list, update, delete, preview or run grooming for any scope its permission tier allowed, regardless of its allowlist. |
