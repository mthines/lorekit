...
| The per-scope catalogs — `memory.scopes` / `GET /memories/scopes`, and the `GET /memories/activity` and `/read-activity` series | Narrowed the same way, inside their RPCs. A scope string is a repo or project name, so an unfiltered catalog would leak exactly what scoping hides — and the two activity series return one row per scope name too, so narrowing only the catalog would move the leak rather than close it. A read whose scope could not be attributed names nothing and still counts toward the total. |
