...
export default function OrganizationTutorialPage() {
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col">
        <TutorialStep number={1} title="Create an organization">
          <p>
            Go to <strong>Settings → Organization</strong> and click{' '}
            <strong>Create organization</strong>. Enter a display name and a URL-safe slug
            (lowercase letters, digits, hyphens). The slug must be globally unique.
          </p>
          <p className="mt-2">
        ...
        <TutorialStep number={2} title="Invite teammates">
          ...
          <ul className="mt-2 list-disc pl-5 space-y-1">
            ...
            <li>A <strong>handle</strong> invite is in-app only — the invitee sees a banner on their Overview.</li>
          </ul>
          <p className="mt-2">
  ...
  org:    "my-team"          // org slug
        ...
        <TutorialStep number={4} title="Bind a scope to your org (optional)">
          ...
          <p className="mt-2">
            Go to <strong>Settings → Shared scopes</strong> → enter a scope string
            (e.g. <code>repo::myteam/api</code>) → click <strong>Bind scope</strong>.
          </p>
          <p className="mt-2">
            After binding, any write under <code>repo::myteam/api</code> by a write-capable
            member routes to the org automatically — no <code>org</code> parameter needed.
            A non-member writing under a bound scope gets a graceful fallback to personal
            lore (never rejected) with a <code>notice</code> explaining it.
          </p>
        </TutorialStep>

        <TutorialStep number={5} title="Browse team lore in the Explorer">
            <p>
              In the <strong>Explorer</strong>, use the <strong>All · Personal · {'{org}'}</strong>{' '}
      ...
      <TutorialCallout>
        ...
        <a href="/tutorials/private" className="text-[var(--color-accent)] underline underline-offset-2">
