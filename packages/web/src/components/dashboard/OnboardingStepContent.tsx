...
function ConnectStep({
  ...
  /** Raw Supabase MCP endpoint — shown as the "Your MCP endpoint" label. */
  mcpUrl: string;
  /** Stable vanity URL (lorekit.io/v1/mcp) — used in copyable config snippets. */
  vanityMcpUrl: string;
  ...
  // Snippets use vanityMcpUrl (lorekit.io/v1/mcp) so users copy the stable URL;
  // the token is injected as --header "Authorization:Bearer <token>" so it never
  // appears in the URL or server request logs.
  const clientConfigs: McpClientConfig[] = MCP_CLIENTS.map((c) => ({
    ...
    filename: c.filename,
    hint: c.hint,
    snippet: c.buildConfig(vanityMcpUrl, activeToken),
  }));
...
interface OnboardingStepContentProps {
  ...
  mcpUrl: string;
  /** Stable vanity URL for config snippets. Defaults to mcpUrl when absent. */
  vanityMcpUrl?: string;
  webhookUrl?: string;
  webhookSecrets?: WebhookSecret[];
  autoGeneratedToken?: string;
...
export function OnboardingStepContent({
  ...
  if (step === 'connect') {
    return (
      <ConnectStep
        mcpUrl={mcpUrl}
        vanityMcpUrl={vanityMcpUrl ?? mcpUrl}
