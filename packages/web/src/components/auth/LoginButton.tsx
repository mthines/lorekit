...
export function LoginButton() {
  ...
  return (
    <button
      onClick={handleLogin}
      disabled={loading}
      className="group relative flex h-11 min-w-[200px] items-center justify-center gap-2.5 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elevated)] px-5 text-sm font-medium text-[var(--color-content-primary)] transition-all duration-200 hover:border-[var(--color-accent)] hover:bg-[var(--color-accent-subtle)] hover:text-[var(--color-accent)] focus-visible:outline-2 focus-visible:outline-[var(--color-accent)] disabled:cursor-not-allowed disabled:opacity-50"
      aria-busy={loading}
    >
      {/* GitHub icon */}
      <svg
