...
export function LessonDetailSheet({ lesson, onClose, onMutated, layout = 'auto' }: LessonDetailSheetProps) {
  ...
  const editForm = useEditableForm<LessonFormValues>({
    ...
    onSave: async (data) => {
      ...
      // panel without requiring a page refresh. `lore-tags` and `lore-facets`
      // are included because an edit can add or remove a label, which changes
      // the filter menu's catalog and its per-value counts; without this they
      // stay stale for the 90s staleTime.
