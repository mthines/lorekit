...
export function LoreExplorer({ scopes, lessons }: LoreExplorerProps) {
  ...
  return (
    <>
      <div className="flex h-full gap-0 overflow-hidden rounded-xl border border-[var(--color-border)]">
          ...
          <div className="flex-1 overflow-y-auto">
            {scopes.length > 0 ? (
              ...
              <EmptyState
                icon={BookOpen}
                title="No scopes yet"
                description="Run an agent to create your first lesson."
              />
            )}
          </div>
        </div>

        {/* Lesson list — right panel */}
