...
*New to LoreKit? Start here → [Give your coding agent a memory in one command](/blog/give-your-agent-a-memory).*
...
Start with the thing that scales: what bounds the block. It's a **character budget** — `hooks.sessionStart.maxChars`, default **3000** (about 25 index lines) — because that's what actually costs you. "15 lessons" is meaningless across scale: fifteen terse keys and fifteen paragraph-long ones differ by an order of magnitude in what they take from the window, and a fixed *count* fails at both ends of the range — it pads a six-lesson store up to a number it doesn't have, and truncates a sixty-thousand-lesson store down to one, silently, with no way to tell which happened.

A budget does neither. Six lessons render six lines and stop. Sixty thousand render however many fit in 3000 characters, ranked best-first. The window cost is flat; the count adapts to the store.

```js
// packages/cli/src/shared/control.mjs — two dials, both defaulting small.
const DEFAULT_SESSION_START_MAX_CHARS   = 3000;  // characters you SEE — a slice of the window
const DEFAULT_SESSION_START_MAX_LESSONS = 100;   // read DEPTH — candidates the ranker chooses from
```
...
```js
// packages/cli/src/shared/lessons-pure.mjs (shape of scoreWithTerms) — every factor in [0,1], weights normalized (default: all equal)
const score =
...
const capped  = capPerBucket(ranked, { cap: loopCap, bucketOf: loopBucketOf });   // loopCap=1
const lessons = diversifyRankedLessons(capped, { ...rankOpts, k: ceiling });      // MMR, λ 0.7 — ceiling = maxLessons
```

A **per-bucket cap** keeps any single self-improvement loop — a bot writing its own `loop::` bookkeeping, which recurs constantly and scores high — from taking the read. One per bucket, and general, human-written lessons are never capped; they're what the room is for. As a store accumulates more automated writers, this is what stops them from crowding out the knowledge a coding session actually needs.
...
<figure>
  ...
  <figcaption>The aw agent and pr-reviewer write constantly; each loop reads its own bucket in full, while a coding session's read caps every loop bucket at one — so your window stays the human-written lessons a task needs. Wire the same loop into your own workflow with lorekit-setup.</figcaption>
</figure>
...
- **Sixty thousand:** you get a 3000-character slice of the highest-signal, de-duplicated, branch-warmed lessons — chosen from up to 100 candidates per scope — plus a map of the four scopes holding the rest and a one-call path (`memory.search`) to any of them.
...
- **Ranking sees each scope's newest window, not all of it.** A scope is read to its newest `maxLessons` memories (100 by default, and never more than the API's 100 per page) and ranked within that window — so on a huge scope, an old-but-high-salience lesson past that depth won't surface in the session-start block. It's still there, and `memory.search` reaches it by content; the read is depth-bounded on purpose so a big store never stalls your session start.
