...
One command scaffolds the `lorekit-memory`, `lorekit-setup` and `lorekit-groom` skills plus the lifecycle hooks into your `.claude/` (project) or `~/.claude/` (global), and writes an MCP server entry — into `.mcp.json` at your repo root for a project install, or `~/.claude.json` for a global one:
...
Then point the MCP server at your own machine instead of the hosted one. `install` merges a `lorekit` entry into that config — leaving any other servers alone — and by default it points at the hosted endpoint. Replace **that entry only** with the CLI's local stdio server:

```json
// mcpServers → edit this entry, leave your others alone
...
**Spike lessons that stay on the spike.** Scopes are `global`, `project::{name}`, `repo::{owner}/{repo}` and `branch::{owner}/{repo}::{branch}`, read in a fixed order: project first, then branch, then repo, then global. Project wins outright; below it, the more specific scope beats the broader one. Write an experiment's findings to the branch scope and they surface on that branch and nowhere else:
...
Delete the branch and the lesson stops being read — nothing prunes it for you, so the file sits under `~/.lorekit/branch/…` until you `rm` it or call `memory.delete`. If the branch merges instead, you rewrite the key at `repo::acme/checkout` and delete the branch copy. `main` never sees the mess. Run `npx @lorekit/cli tree` to see which scope wins a duplicated key before a task starts.
