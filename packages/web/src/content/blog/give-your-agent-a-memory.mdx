...
description: "Your agent re-solves yesterday's problem because nothing wrote it down. The fix isn't a platform you adopt — it's one command, a folder you own, and a loop that closes on your own disk. And when you're ready for a team, the same read path merges in a hosted store. No migration."
date: "2026-08-15"
author: "Mads Thines"
readingMinutes: 6
...
And here's the part that makes it more than a starter toy: **the same read path runs over a local directory or a hosted Postgres — scaling local→remote is additive, a *merge* rather than a migration.** You start with a folder. You never have to leave it behind.
...
Then point the MCP server at your own machine instead of the hosted one. `install` writes the hosted entry by default, so swap it for the CLI's local stdio server:
...
<figure>
  <img src="/blog/local-to-shared.svg" alt="A scaling strip in three stages. Today: a local store of plain markdown files in a folder you own. Then: you add a connection — the LOREKIT_MCP_URL and LOREKIT_TOKEN environment variables, and mode set to remote in .lorekit.json. Result: the local folder and a hosted Postgres store both feed one unchanged read path, and the CLI reads both side by side. No export, no import, no sync process." width="720" height="300" />
  <figcaption>The read path never changed — it just has more inputs. Local was never a toy; it's the base layer everything else stacks on.</figcaption>
</figure>
