...
What a filter is checked against is the separator, the prefix and the charset — not the finer per-prefix shape rules the write path applies. `?scope=repo` and `?scope=team::acme` are a **400**; `?scope=repo::noslash` is accepted and simply matches nothing, because tightening that would reject scopes already in the store.
