...
If an admin has bound a scope (e.g. `repo::myteam/api`) — or a **wildcard prefix** such as `repo::myteam/*` covering every repo under that owner — to an org (see [Settings → Organization → Shared scopes](/settings/organization)):
