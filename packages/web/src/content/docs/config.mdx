...
| `hooks.sessionStart.maxChars` | `number (200–20000)` | Both | Character budget for that block. Defaults to 1500 (~375 tokens). Out-of-range values are clamped rather than rejected. A second, non-configurable bound caps the block at 40 memories however many characters the budget funds, so raising maxChars past a few thousand buys longer lines rather than more of them. Repo wins over user — a repo that declared a budget owns the decision even when the value is unparseable, so a typo degrades to the default instead of silently handing the choice to a personal config. |
