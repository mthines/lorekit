...
| `hooks.sessionStart.maxLessons` | `number (3–200)` | Both | How many memory LINES that block may hold, where `maxChars` bounds its characters — whichever binds first decides the block, so raising this alone changes nothing unless `maxChars` comes up with it. Defaults to 40, which is the ceiling LoreKit has always applied, so leaving it unset is exactly the behaviour you had. Values are clamped, not rejected, with the same declared-value-owns-the-layer rule as `maxChars`. Raising it above 40 also raises the per-scope candidate fetch to match (25 memories per scope by default, 80 at `"maxLessons": 80`, and never more than 100 — the largest page the API will return) — that read happens on every session start, so a bigger index costs a bigger read. Keep it small unless you specifically want a wider index: the block is meant to be a working set, with `memory.search` for the tail. |
...
A second bound holds it from the other direction: `maxLessons`, the number of **lines** the block may hold, default 40. The budget alone cannot stop a store of 500 one-word keys from rendering 400 lines inside it, and a 400-line index is unreadable however few characters it costs. Whichever of the two binds first decides the block, so the two are raised together or not at all.
...
**Raising the line ceiling raises that fetch too.** A ceiling the read cannot feed would be a dial that visibly does nothing on a workspace whose lore lives in one scope, so setting `"hooks.sessionStart.maxLessons": 80` reads 80 memories per scope instead of 25 (and the map's marker moves with it — `80+`). That read happens on **every session start**, against files on disk in local mode or over the network in remote mode, which is the trade: a wider index costs a slower start. The fetch only ever grows — leaving `maxLessons` at its default, or lowering it, keeps the per-scope read at exactly 25, so an unconfigured workspace pays nothing for this setting existing and a lowered one still lets the ranking choose from a full pool.

It also stops at **100 per scope**, which is the largest page `GET /memories` will return. So a ceiling above 100 still reads 100 from each scope and fills the remaining lines from the others — four scopes offer 400 candidates for a 200-line ceiling, which is why the ceiling can exceed the page size without the block coming up short.

```json
// .lorekit.json — a deliberately wide index, for a large store
{
  "hooks.sessionStart.maxChars": 12000,
  "hooks.sessionStart.maxLessons": 80
}
```

Both are needed: at the default `maxChars` of 1500 the budget runs out long before line 80, so raising `maxLessons` on its own would change nothing.
