...
- **`hybrid`** (default) — fill the budget, then add one line naming what was left out and where it lives:
  ...
  ```text
  LoreKit: 9 of 143 memories loaded · repo::acme/widget — considerations, not rules; read any in full with memory.read.
  ...
  More lore: repo::acme/widget 118 · global 25 — memory.search or memory.read to drill in.
  ```
...
The header reads `9 of 143` whenever the block is truncated, so an agent can tell that reaching for `memory.search` is worth it. A count in the map ending in `+` (`118+`) is a lower bound — that scope holds at least that many.
