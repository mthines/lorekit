...
This starts the local stdio MCP server. Point your agent at it instead of the hosted endpoint — this is the entry that makes an offline setup actually offline:

```json
// .mcp.json — the local store, no endpoint and no token
{
  "mcpServers": {
