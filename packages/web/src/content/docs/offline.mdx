...
order: 2
keywords: ["offline", "local store", "cli", "install", "doctor", "mcp", "memory.write", "scope", ".lorekit.json", "deny", "mode", "search", "migrate"]
---
...
**Next steps:** once you have offline lore working, you can add the remote hosted store for cross-machine access — and push the lessons you already accumulated up to it with one `migrate` command. See the [Remote storage tutorial](/docs/remote) to continue.
