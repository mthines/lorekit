...
Offline storage is the fastest way to start, and moving to the hosted store later is additive rather than a migration: add a `LOREKIT_MCP_URL` and `LOREKIT_TOKEN`, then switch `mode` to `remote` (Step 6 — the committed `.lorekit.json` outranks those variables, so it has to agree). Nothing is exported and your local files stay exactly where they are; `npx @lorekit/cli list` keeps showing both stores, side by side.
...
This starts the local stdio MCP server. Point your agent at it instead of the hosted endpoint — this is the entry that makes an offline setup actually offline. `install` merges its `lorekit` entry into `.mcp.json` alongside any other servers you have, so edit **that entry only** rather than replacing the file:
