...
Offline storage is the fastest way to start, and moving to the hosted store later is additive rather than a migration: add a `LOREKIT_MCP_URL` and `LOREKIT_TOKEN` — those are connection settings, not a mode — and then switch `mode` to `remote` in the `.lorekit.json` from Step 6, which is what actually selects it. Nothing is exported and your local files stay exactly where they are; `npx @lorekit/cli list` keeps showing both stores, side by side.
...
This scaffolds the `lorekit-memory`, `lorekit-setup`, and `lorekit-groom` skills into your `.claude/` (project) or `~/.claude/` (global) directory and writes an MCP server entry — into `.mcp.json` at the repo root for a project install, or `~/.claude.json` for a global one — then asks whether to wire the lifecycle hooks.
