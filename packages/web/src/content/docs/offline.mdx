...
Offline storage is the fastest way to start. You can migrate to remote storage later by adding a `LOREKIT_MCP_URL` and `LOREKIT_TOKEN` — the CLI reads both stores and merges them automatically.
