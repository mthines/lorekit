...
order: 2
keywords: ["offline", "local store", "cli", "install", "doctor", "mcp", "memory.write", "scope", ".lorekit.json", "deny", "mode", "search", "serve", "dashboard"]
---
...
<TutorialStep number={6} title="Browse your offline lore in the dashboard">

```bash
npx @lorekit/cli serve
```

Runs this same dashboard **locally, against your local `.lorekit/` store** — no Supabase project, no account. It starts a small local server that speaks the dashboard's own API over your offline files, launches the dashboard pointed at it, prints both URLs, and opens your browser. Covers the Lore Explorer and scope tree: list, filter, search, view, edit, archive, and restore — read **and** write, straight to your `.lorekit/*.md` files. Press <kbd>Ctrl-C</kbd> to stop.

The alias `lorekit web` does the same thing. See [Local web dev mode](https://github.com/mthines/lorekit/blob/main/docs/local-web.md) for the full command reference.

</TutorialStep>

<TutorialStep number={7} title="(Optional) Commit a project config file">
