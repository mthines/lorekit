...
| `filters` | `null` | Filter bar conditions across six dimensions — label, agent, trigger, repo, branch, pull request (e.g. `[{"field":"branch","operator":"is","values":["main"]}]`). Wins over `tags`, including when empty. |
| `tags` | `[]` | Legacy label shorthand, AND across labels (e.g. `["perf","ci"]`). |
