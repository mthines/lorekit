...
| The 3D memory map | `/lore?view=%22map%22` |
...
| `view` | `null` | Which view of the results to open — `"map"` for the 3D memory map, absent for the list. The list is never written, so a plain `/lore` link is unchanged. |
