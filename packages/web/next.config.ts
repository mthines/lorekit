...
const nextConfig: NextConfig = {
  ...
  // action ID to the NEW deployment, which 404s it — the Lore Explorer's
  // Accept / Decline / onboarding buttons go silently dead until a hard reload.
  //
  ...
  // `/dashboard` and `/overview` are both old names for the app's home
  // route, which is now the Lore Explorer at `/lore`. `/dashboard` predates
  // the "Overview" rename; `/overview` was the page itself, deleted once its
  // onboarding + first-token-mint duties moved to `/lore`. Both paths are in
  // bookmarks, docs, invite emails already delivered, and any link shared
  // before the renames, so they redirect rather than 404. Not permanent
  // (308): a browser caches a 308 indefinitely, and either path may be
  // reused later.
  //
  ...
  async redirects() {
    return [
      { source: '/dashboard', destination: '/lore', permanent: false },
      { source: '/overview', destination: '/lore', permanent: false },
