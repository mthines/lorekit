...
export const MCP_TOOLS = [
  ...
  },
  {
    ...
    inputSchema: { type: 'object', properties: {} },
    returns: '`{ "entries": [{ "id", "scope", "name", "mode", "enabled", "min_age_days", "unseen_days", "max_seen_count", "created_at", "updated_at" }] }`',
  },
  {
    name: 'policy.create',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        unseen_days: { type: 'integer', minimum: 1, maximum: 3650, description: 'Match lessons not individually opened via MCP or the CLI for at least this many days (a bulk list/search result or a dashboard view does not count). A never-opened lesson always matches.' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        unseen_days: { type: 'integer', minimum: 1, maximum: 3650, description: 'Match lessons not individually opened via MCP or the CLI for at least this many days (a bulk list/search result or a dashboard view does not count). Omit to leave unchanged; pass explicit null to clear.' },
