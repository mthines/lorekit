...
match (25 memories per scope by default, 80 at `"maxLessons": 80`, capped at the
API's 100-per-page maximum), which every session start pays for; at or below the
default the fetch is unchanged. A header
