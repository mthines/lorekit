...
**The filter grammar is the separator, prefix and charset rules above, not the per-prefix shape rules.** The edge validator the filters call is deliberately lighter than the write-side one, so `?scope=repo::noslash` and `?scope=repo::` are accepted as filters and simply match nothing, while `?scope=repo` (no separator) and `?scope=team::acme` (unknown prefix) are a `400`. Tightening the filter grammar would reject scopes that already exist in the store, so it is a separate change.
