...
export const MCP_TOOLS: readonly McpToolDoc[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        embedding: {
          ...
          items: { type: 'number' },
          description:
            'Optional pre-computed embedding vector, exactly 1536 numbers. Supply it to store an embedding you generated yourself; the server does not call any provider. Must be sent together with `embedding_model`.',
        },
        embedding_model: {
          type: 'string',
          description:
            'The model that produced `embedding`, e.g. `text-embedding-3-small`. Required whenever `embedding` is supplied, and rejected on its own.',
        },
      },
    },
    returns:
    ...
    notes: [
      ...
      '**Bring-your-own embedding (`embedding` + `embedding_model`):** Embedding is off by default because it costs money per write. Supplying a vector you computed yourself opts that write in at no cost to the server — there is no separate flag, and it works even on a deployment with no embedding provider configured. Both fields are required together (a vector with no model name cannot be compared against others or selectively re-embedded), and the vector must be exactly 1536 finite numbers. Storing it is best-effort and never fails or slows the write; an invalid pair is rejected up front rather than silently dropped.',
    ],
  },
  {
    name: 'memory.read',
