...
describe('tier coverage vs the action gate', () => {
  ...
  // shortest retention. `classifyWebhookAction` itself lives in the production
  // edge webhook handler (`supabase/functions/mcp/webhook.ts`), which cannot
  // cross-import mcp-core, so this list is kept in sync with it by hand rather
  // than by a shared-importable assertion.
