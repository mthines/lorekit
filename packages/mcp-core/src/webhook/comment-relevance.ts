 ...
 *
 * This module is the tested source of truth.  It is mirrored (self-contained,
 * no cross-package import) into supabase/functions/mcp/comment-relevance.ts
 * for the Deno edge function — the same pattern used for
 * webhook-secret-select.ts and webhook-installation.ts.  Keep the two in sync
 * when either changes (guarded by edge-parity.spec.ts).
 */
