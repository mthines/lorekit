...
--   api_tokens.*               three additive columns (kind, expires_at,
--                              client_id) so an OAuth-issued credential is
--                              just an api_tokens row
--
-- DESIGN NOTES
-- ------------
...
--   * The org allow-list the consent screen produces reuses `api_tokens`'
--     EXISTING `scopes`/`org_access`/`org_ids` tri-state columns (00068
--     `api_token_scoping.sql`, enforced by 00069) rather than adding a second,
--     OAuth-specific scoping mechanism — this migration adds no org_ids column
--     of its own. The consent screen's per-org checkboxes map onto
--     `org_access = 'selected'` + `org_ids = <ticked orgs>`; leaving every box
--     unticked (personal lore only) maps onto `org_access = 'personal'`. There
--     is no consent-screen affordance for `org_access = 'all'` (every org,
--     including ones joined after authorizing) — an OAuth-issued token is
--     therefore always `personal` or `selected`, never `all`, which is the
--     least-privilege default `KeyRestriction`'s tri-state already supports.
--     `_shared/auth/tenant-scope.ts`'s `effectiveOrgIds`/`applyTenantScope` and
--     the `memory_write`/`memory_delete` RPCs' `p_key_org_access`/`p_key_org_ids`
--     parameters enforce it identically to a scoped dashboard token, with zero
--     OAuth-specific code on the read or write path.
...
-- All three are nullable/defaulted so every existing row keeps its exact
-- current meaning: kind='personal', no expiry, no client. Org scoping reuses
-- 00068's `org_access`/`org_ids` columns — see the DESIGN NOTES above — so
