 ...
 *
 * Self-contained mirror of packages/mcp-core/src/webhook/github-review-parse.ts,
 * which is the tested source of truth: its spec covers every field rule here,
 * and packages/mcp-core/src/edge/edge-parity.spec.ts compares the two files
 * byte-for-byte (comments and blank lines stripped) so they cannot drift.
 * Edit the mcp-core copy, then re-copy it here — never patch this file alone.
 * The duplication buys the edge function a zero-import module, which is what
 * keeps the Deno bundle free of a cross-package dependency.
 */
