 ...
 *
 * MIRROR of `packages/mcp-core/src/telemetry/mcp-client-attribute.ts` — the
 * edge function cannot cross-import the Node package, so this copy is verbatim
 * and held to it by `edge-parity.spec.ts` (registered in
 * `packages/cli/src/shared/mirror-pairs.mjs`). Change the mcp-core copy first,
 * then copy it here — the `scope-type-attribute.ts` pattern.
 */
