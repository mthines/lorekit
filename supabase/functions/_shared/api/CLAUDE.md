...
- `isOriginAllowed(allowed, origin)` — true when the expanded allowlist contains `*` or the exact
  ...
  the label being `lorekit` or `lorekit-…` — so `lorekit.vercel.app` and every preview host
  `lorekit-git-<branch>-<scope>.vercel.app` match, while `lorekit-x.attacker.vercel.app`,
  `lorekitten.vercel.app` and `lorekit.io` do not). Both are admitted independently of
